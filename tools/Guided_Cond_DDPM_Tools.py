...
class Data_Normalizer:
    ...
    def fit_Data(self,X): # tansfer data value into [-1, 1]
...
class Drag_Regression_ResNet(torch.nn.Module):  # Predictor for wave drag 
    def __init__(self, Reg_Dict):
        nn.Module.__init__(self)
...
class Denoise_ResNet_Model(torch.nn.Module):
    def __init__(self, DDPM_Dict):
        ...
        self.finalLayer = nn.Sequential(nn.Linear(self.tdim, self.xdim))    # 128 - 44
        
    
        self.X_embed = nn.Linear(self.xdim, self.tdim)  # 44 - 128
        

        self.Con_embed = nn.Sequential(                         # the embemdding of conditions
        ...
        self.time_embed = nn.Sequential(                       # the embemdding of timestep
            nn.Linear(self.tdim, self.tdim),
            nn.SiLU(),
            nn.Linear(self.tdim, self.tdim))
    ...
    def LinLayer(self, dimi, dimo):
        ...
        return nn.Sequential(nn.Linear(dimi,dimo),
                             nn.SiLU(),                 # sigmoid function + linear 
                             nn.BatchNorm1d(dimo),      # Batch normalized 
                             nn.Dropout(p=0.1))         # 随机将部分参数置零进行训练 提高鲁棒性
    ...
    def forward(self, x, cons, timesteps):
        ...
        x = self.X_embed(x) + self.time_embed(timestep_embedding(timesteps, self.tdim)) + self.Con_embed(cons)  # the input of x, timestep, and condition 
        res_x = x
        
        for i in range(0,len(self.fc)):
            x = self.fc[i](x)
    
        x = torch.add(x,res_x)      # ResNet 残差网络
...
class GuidedDiffusionEnv:
    ...
    def __init__(self, DDPM_Dict, Class_Dict, Drag_Reg_Dict,LOA_wBulb_Reg_Dict, WL_Reg_Dict,Vol_Reg_Dict, X, X_neg, VolVec, BOAVec, DdVec, Cw = 0):   
        self.DDPM_Dict = DDPM_Dict  # Cshipgen
        ...
        # Set Up Design Data
        self.X = self.data_norm.fit_Data(X) # normalize data into [-1, 1]
        self.X_np = self.X 
        ...
        self.draft = [.25, .33, .50, .67]
        Froude_N = np.arange(0.15, 0.45, .05 ) 
        self.Cw = Cw 
        ...
        self.optimizer_diffusion = torch.optim.AdamW(self.diffusion.parameters(), lr=lr, weight_decay=weight_decay)     # Diffusion Model 
        self.optimizer_classifier = torch.optim.AdamW(self.classifier.parameters(),lr=.001, weight_decay=weight_decay)  # Classifier
    ...
    def extract(self,a, t, x_shape):        
        b, *_ = t.shape
        t = t.to(a.device)
        out = a.gather(-1, t)               # extract data from a refer to t 
        while len(out.shape) < len(x_shape):
            out = out[..., None]            # adding None at the end of tensor 
        return out.expand(x_shape)
    
    def _anneal_lr(self, epoch_step):       # kind like simulated annealing
    ...
    def batch_train(self, batch_size=None):
        ...
        A = A = np.random.randint(0,self.datalength,batch_size)     # Sample indictors of design
        ...
        Vol = np.array([self.interp(self.VolVec[A[i]],self.T_vec,t[i]) for i in range(0,len(t))]) #Non-dimensionalized Waterline Length, the volume of ith waterline of ith ship
        Dd = self.DdVec[A]  
        BOA = self.BOAVec[A]
        
        x_batch = self.X[A]     # Extract training data
    ...
    def Ct_batch_train(self, batch_size=None):
        '''
        This function takes in a batch of design vectors and outputs the corresponding CT values
        '''
        if batch_size == None:
        ...
        A = np.random.randint(0,self.datalength,batch_size)     # Sample indictors of design
        
        #Random Waterline and LOA and velocity 
        t = np.random.uniform(self.T_range[0], self.T_range[1], (batch_size,)) # self.T_range = [.25,.67]
        Fn = np.random.uniform(self.Fn_range[0], self.Fn_range[1], (batch_size,)) # self.Fn_range = [.15,.45]
        Log_LOA = np.random.uniform(self.Log_LOA_range[0], self.Log_LOA_range[1], (batch_size,)) # self.Log_LOA_range = [.47,2.65]
        LOA = 10**Log_LOA

        drag_cond = np.concatenate((t[:,np.newaxis],Fn[:,np.newaxis],Log_LOA[:,np.newaxis]),axis=1) # Drag_conditioning is [ToD, U(m/s), LOA (m)]
        drag_cond = torch.from_numpy(drag_cond.astype('float32')).to(self.device)

        x_batch = self.X[A]     # Extract training data

        x = x_batch
        x = torch.cat((x,drag_cond),dim=1) #concatenate for drag prediction  [x, t, fn, log10LOA]
        Ct = 10**self.Drag_Reg(x)

        Ct = Ct.to(torch.device('cpu')).detach().numpy()

        cond_batch = np.concatenate((t[:,np.newaxis],Fn[:,np.newaxis],LOA[:,np.newaxis], Ct),axis=1) # Drag_conditioning is [ToD, U(m/s), LOA (m), Drag ] 
    ...
    def q_sample(self,x_start, t, noise=None):      # forward process 
        ...
        """
        if noise is None:
            noise = torch.randn_like(x_start).to(self.device) # generate normal distribution random values with same input size 
    
        sqrt_alphas_cumprod_t = self.extract(self.sqrt_alphas_cumprod, t, x_start.shape)    # alpha_hat^0.5
        sqrt_one_minus_alphas_cumprod_t = self.extract(                                     # (1- alpha_hat)^0.5
            self.sqrt_one_minus_alphas_cumprod, t, x_start.shape
        )
    
        return sqrt_alphas_cumprod_t * x_start + sqrt_one_minus_alphas_cumprod_t * noise

    def q_Resample(self,x_start, t, noise=None):      # forward process 
        """
        qResample
        """
        if noise is None:
        ...
        sqrt_alphas_cumprod_t = self.extract(self.sqrt_alphas_cumprod, t, x_start.shape)    # alpha_hat^0.5
        one_minus_alphas_cumprod_t = self.extract(                                     # 1- alpha_hat
            self.one_minus_alphas_cumprod, t, x_start.shape
        )
    ...
    def p_loss(self,x_start, cond, t, noise=None,loss_type='l2'):
        ...
        '''
        if noise is None:
            noise = torch.randn_like(x_start)                        # X_start -> X_0 or GT 
        
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)   # Expected noised X_t+1
        predicted_noise = self.diffusion(x_noisy, cond, t)           # Predicted e_t   
    ...
    def run_diffusion_step(self, x,cond):
        self.optimizer_diffusion.zero_grad()        # Reset the gradients to zero
        
        t = torch.randint(0,self.timesteps,(self.batch_size,),device=self.device) # timestep
        loss1 = self.p_loss(x,cond, t,loss_type='l2')
        
        loss = loss1 
        loss.backward()                     # calculate gradient
        self.optimizer_diffusion.step()     # update the weights    
    ...
    def run_train_diffusion_loop(self, batches_per_epoch=100):
        ...
        num_batches = self.datalength // self.batch_size         # split sample into each batch
        ...
        for i in tqdm(range(self.num_diffusion_epochs)):
            ...
            for j in range(0,batches_per_epoch):
                ...
                # x_batch, cond_batch = self.batch_train()    # extra the training data's indicator and their condition label 
                x_batch, cond_batch = self.Ct_batch_train()

                loss = self.run_diffusion_step(x_batch, cond_batch)     # calculate the loss 
                '''
            ...
            self._anneal_lr(i)      # update the learning rate when training done in each batch
            ...
            self.ema.update(self.diffusion)     # Exponential Moving Average, smooth the parameter update
    ...
    def Drag_fn(self, x, drag_cond, g=9.81):
        ...
        with torch.enable_grad():
            ...
            # grad = torch.autograd.grad(perf.sum(), x_in)[0] # gradient caculation  
            grad = -2.0*(torch.log10(Ct) - perf)*torch.autograd.grad(perf.sum(), x_in)[0]
    ...
    @torch.no_grad()
    def drag_p_sample(self, x, t, cons):
        time= torch.full((x.size(dim=0),),t,dtype=torch.int64,device=self.device)
        ...
        if t == 0:
            ...
            noise = torch.randn_like(x,device=self.device)
            # Dot product gradient to noise
            return model_mean + torch.sqrt(posterior_variance_t) * (noise*(1.0-self.gamma) + self.gamma*fease_grad.float() - self.lam*drag_grad.float())
    ...
    def vol_drag_p_sample(self, x, t, geom_cons, drag_cons):
        ...
        fease_grad = self.fease_fn(x)
        # print("fease_grad: ", fease_grad)

        drag_grad = self.Drag_fn(x,drag_cons)

        # l1 = self.lam[0]*drag_grad.float()
        # print("drag_grad: ", drag_grad)

        vol_grad = self.Vol_fn(x,geom_cons)
        # print("vol_grad: ", vol_grad)
        #print(gradient.detach().to('cpu')[0])
    ...
    @torch.no_grad()
    def gen_cond_samples(self, cons, x_gt = 0, keep_mask = 0, Repaint = 0):
        ...
        Resample_t = 20
        masks = keep_mask.repeat(num_samples, 1)
        print("x_gt_norm:", x_gt)

        for i in tqdm(range(self.timesteps - 1, 0, -1)):
            if Repaint:
                time = torch.full((x_gt.size(dim=0),), i, dtype=torch.int64,device=self.device)
                betas_t = self.extract(self.betas, time -1, x_gt.shape)
                squrt_one_minus_beta = torch.sqrt(1.0 - betas_t)
                # Resampling
                for k in range(0, Resample_t):
                    # adding noise into the ground truth part
                    if i > 1: 
                        x_gt_t = self.q_Resample(x_start = x_gt, t=time) # Known X_t
                        noise = torch.randn_like(x_gen,device=self.device)
                    else:
                        x_gt_t = x_gt
                        noise = 0

                    x_gen = self.p_sample(x_gen, i,cons) # Unknown X_t
                    # Combine
                    x_gen = x_gen * (1 - masks) + x_gt_t * masks
                    
                    if i > 1 or k < (Resample_t-1) :
                        x_gen = x_gen * squrt_one_minus_beta + noise * betas_t
                    # else:
                        # print('k:', k)
                        # print('i:', i)
                        # temp = x_gt_t * masks
                        # print("gt masked: ", temp[0])
            else:
    ...
    def gen_low_drag_samples(self, Drag_COND, x_gt = 0, keep_mask = 0, Repaint = 0, Resample_t = 20):
        #Drag_COND is a numpy array of the drag conditioning. Each Row is [ToD, Fn(m/s), LOA (m), Ct]
        num_samples = len(Drag_COND)
        ...
        for i in tqdm(range(self.timesteps - 1, guidance_step, -1)):
            if Repaint:
                time = torch.full((x_gt.size(dim=0),),i,dtype=torch.int64,device=self.device)
                betas_t_minus_one = self.extract(self.betas, time-1, x_gt.shape)
                ...
                for k in range(0, Resample_t):
                    ...
                    x_gen = self.drag_p_sample(x_gen, i, drag_cons) # Unknown X_t

                    # Combine
                    x_gen = x_gen * (1 - masks) + x_gt_t * masks
                    
                    if i > 1 or k < (Resample_t-1):
                        x_gen = x_gen * squrt_one_minus_beta + noise * betas_t_minus_one  

            else:
                x_gen = self.drag_p_sample(x_gen, i, drag_cons)

        for i in tqdm(range(guidance_step, 0, -1)):
            if Repaint:
                time = torch.full((x_gt.size(dim=0),),i,dtype=torch.int64,device=self.device) # t 
                betas_t_minus_one = self.extract(self.betas, time-1, x_gt.shape)  
                squrt_one_minus_beta = torch.sqrt(1.0 - betas_t_minus_one)    # 1-
                # Resampling
                for k in range(0, Resample_t):
                    if i > 1: 
                        x_gt_t = self.q_Resample(x_start=x_gt, t=time) # Known X_t
                    ...
                    x_gen = self.p_sample(x_gen, i, drag_cons) # Unknown X_t
                ...
                x_gen = self.p_sample(x_gen, i, drag_cons)
    ...
    def gen_vol_drag_guided_samples(self, Geom_COND, Drag_COND, x_gt = 0, keep_mask = 0, Repaint = 0):
        ...
        x_gen = torch.randn((num_samples,self.diffusion.xdim),device=self.device)   # Sample a random noise as the start point
        ...
        for i in tqdm(range(self.timesteps - 1, guidance_step, -1)):
            ...
            x_gen = self.drag_p_sample(x_gen, i,geom_cons, drag_cons) # vol_drag_p_sample
            ...
            if Repaint:
                ...
                betas_t = self.extract(self.betas, time, x_gt.shape)
                squrt_one_minus_beta = torch.sqrt(1.0 - betas_t)
                # Resampling
                for k in range(0, Resample_t-1):

                    x_gt_t = self.q_Resample(x_start=x_gt, t=time)
                    # Combine
                    x_gen = x_gen * (1 - masks) + x_gt_t * masks
    ...
    def gen_drag_guided_samples(self, Geom_COND, Drag_COND, x_gt = 0, keep_mask = 0, Repaint = 0):
        #Geom_COND is a numpy array of the geometric conditioning. Each row is [ToD, BoL, DoL, Vol]
        #Drag_COND is a numpy array of the drag conditioning. Each Row is [ToD, U [m/s], LOA [m]]
        num_samples = len(Geom_COND)
        ...
        for i in tqdm(range(self.timesteps - 1, guidance_step, -1)):
            ...
            if Repaint:
                time = torch.full((x_gt.size(dim=0),),i,dtype=torch.int64,device=self.device)
                noise = torch.randn_like(x_gen,device=self.device)
                x_gt_t = self.q_sample(x_start=x_gt, t=time)
                betas_t = self.extract(self.betas, time, x_gt.shape)
                squrt_one_minus_beta = torch.sqrt(1.0 - betas_t)
                # Resampling
                for k in range(0, Resample_t-1):
    ...
    def Predict_Drag(self,x,drag_cond, rho=1025.0, g=9.81, UorF = 0):
        #x is a numpy array of the normalized design vector (alread)
        #drag_cond is a numpy array of the drag conditioning. Each Row is [ToD, U[m/s], LOA] #Drag_conditioning is [ToD, Fn(m/s), LOA (m), Ct]
        ...
        x = torch.cat((x,drag_cond[:,0:1]),dim=1) #concatenate ToD for WL Prediction [x, t]
        # print("UORF: ", UorF)
        ...
        if UorF == 0:
            ...
            Fn_cond = drag_cond[:,1:2]
            U =   Fn_cond * torch.sqrt(g* LOA*self.WL_Reg(x))
        
        x = torch.cat((x,Fn_cond, torch.log10(LOA)),dim=1) #concatenate for drag prediction  [x, t, fn, log10LOA]

        # print("Input of Drag_Rage: ", x.shape)
        ...
        Drag = CT*0.5*rho*(U**2)*LOA**2 #Calculate Drag: RT = CT*0.5*rho*U^2*LOA^2

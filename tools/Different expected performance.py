...
for Pa_ind in range(4,5):
    ...
    for HL in range(4):
        ...
        plt.savefig( Address + "Case"+ casename +".jpg", bbox_inches='tight', dpi=300)  # 保存为 JPEG
        ...
        np.save('./result/' + Study_Label + "case " + casename + '_Conditioning_Only_RT_DesVec.npy',X_gen_cond[valid_idx_1])
        np.save('./result/' + Study_Label + "case " + casename + '_Drag_Guidance_DesVec.npy',X_gen[valid_idx])
        np.save('./result/' + Study_Label + "case " + casename + '_Ct_pred.npy',CT[valid_idx])

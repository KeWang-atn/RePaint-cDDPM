...
def plot(precision_recall_pairs, labels=None, out_path=None,
  ...
  plot_handle.tick_params(axis='both', which='major', labelsize=8)
  
  if len(precision_recall_pairs) > 6:
    Selected_indices = np.random.choice(np.arange(1, len(precision_recall_pairs)), 6, replace=False)
    Selected_indices = np.concatenate(([0], Selected_indices))
    precision_recall_pairs = [precision_recall_pairs[i] for i in Selected_indices]
    labels = [labels[i] for i in Selected_indices] if labels is not None else None

...
# Peak_values = np.load('D:/CShipGen/C_ShipGen-main/data/peak_values.npy', allow_pickle=True)
# print("Peak values: ", Peak_values)

# Rare_values = np.load('D:/CShipGen/C_ShipGen-main/data/rare_values.npy', allow_pickle=True)
...
Study_Label = 'Study_' + Labels[0] + '_CT_' + str(Ships[0, 4]) + '_Fn_' + str(Ships[0, 5])
...
0.00000000e+00,  0.00000000e+00,  0.00000000e+00,  0.00000000e+00,
0.00000000e+00,  0.00000000e+00,  0.00000000e+00,  0.00000000e+00,
0.00000000e+00], dtype=torch.float32).to('cuda:0')
...
results = []
result = []
for Pa_ind in range(10,19):
    # Reset the mask and configure new one
    keep_mask = torch.zeros(keep_mask.shape).to('cuda:0')
    keep_mask[Pa_ind] = 1 

    for HL in range(11):
        # Reload expected Ct value
        x_gt =x_gt_unorm.cpu().numpy()
        diff = T2.data_norm.X_UL_Scaled[0, Pa_ind] - T2.data_norm.X_LL_Scaled[0, Pa_ind]
        value = T2.data_norm.X_LL_Scaled[0, Pa_ind] + diff/10 * (HL)
        # print("value: ", value)
        x_gt[Pa_ind] = value
        print(f"P {Pa_ind} value: ", value)

        x_gt = 2.0*(x_gt-T2.data_norm.X_LL_Scaled)/(T2.data_norm.X_UL_Scaled- T2.data_norm.X_LL_Scaled) - 1.0 # transfer value into -1 to 1 
        x_gt = T2.data_norm.normalizer.transform(x_gt)
        x_gt = torch.from_numpy(x_gt.astype('float32')).to('cuda:0')
        ...
        axs[1].set_xlabel('Ct value')
        axs[1].set_ylabel('Density')
        axs[1].legend()

        plt.tight_layout()
        Address = "./result/image/"
        plt.savefig( Address + "Case"+ str(casename) +" Val "+ str(pval)+".jpg", bbox_inches='tight', dpi=300)  # 保存为 JPEG
        ...
        np.save('./result/' + Study_Label + " case " + str(casename) + " Val "+ str(pval) + '_Conditioning_Only_RT_DesVec.npy',X_gen_cond[valid_idx_1])
        np.save('./result/' + Study_Label + " case " + str(casename) + " Val "+ str(pval) + '_Drag_Guidance_DesVec.npy',X_gen[valid_idx])
        np.save('./result/' + Study_Label + " case " + str(casename) + " Val "+ str(pval) + '_Ct_pred.npy',CT[valid_idx])
